      ...
      >
        ...
        >
          ...
          <div className="relative w-full md:w-80 font-sans">
            ...
            <input
              type="text"
              placeholder="Search deployments, techs..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-3 bg-white/[0.03] border border-white/10 rounded-xl text-xs text-white placeholder-white/30 focus:outline-none focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]/20 transition-all duration-300 font-extrabold"
            />
          </div>

          {/* Category Filter Pills */}
          <div className="flex flex-wrap gap-1.5 w-full md:w-auto overflow-x-auto p-1 bg-white/[0.03] rounded-full border border-white/10 backdrop-blur-md">
            {categories.map((cat) => (
              <button
                key={cat}
        ...
        {filteredProjects.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProjects.map((project, idx) => (
              <motion.div
                ...
                className="group glass-panel hover:border-white/20 hover:shadow-[0_12px_40px_rgba(56,189,248,0.06)] rounded-2xl overflow-hidden flex flex-col h-full transition-all duration-500 cursor-pointer text-left hover:scale-[1.015] border border-white/5 relative card-shine"
              >
                {/* Cover Image */}
                <div className="relative aspect-video overflow-hidden bg-[#050B14]">
                  <img
                    ...
                    className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-700 opacity-70 group-hover:opacity-85"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#02122c] via-transparent to-transparent opacity-90" />
                  <span className="absolute top-4 left-4 px-2.5 py-1 rounded bg-black/60 border border-white/10 text-[9px] font-mono text-[#38BDF8] font-extrabold tracking-widest uppercase shadow-sm">
                    {project.category}
                  </span>
                </div>
                ...
                <div className="p-6 flex flex-col justify-between flex-1">
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="text-white/60 text-xs leading-relaxed line-clamp-3 font-sans font-medium">
                      {project.description}
                    </p>
                  </div>
                  ...
                  <div className="mt-8 pt-4 border-t border-white/5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {project.technologies.slice(0, 4).map((tech, tIdx) => (
                        <span key={tIdx} className="px-2 py-0.5 bg-white/[0.03] text-[9px] text-white/70 font-mono rounded-md border border-white/5 font-semibold">
                          {tech}
                        </span>
                      ))}
                      {project.technologies.length > 4 && (
                        <span className="px-2 py-0.5 bg-white/[0.03] text-[9px] text-white/40 font-mono rounded-md border border-white/5 font-bold">
                    ...
                    <div className="mt-6 flex items-center justify-between text-xs font-mono">
                      <span className="text-[9px] text-white/40 uppercase tracking-widest font-extrabold">Inspect Specs</span>
          ...
          <motion.div variants={itemVariants} className="text-center py-20 bg-white/5 border border-white/10 border-dashed rounded-3xl flex flex-col items-center justify-center gap-4">
            ...
            <button
              ...
              className="px-4 py-2 bg-white text-[#050B14] text-xs font-mono hover:bg-white/90 rounded-full transition-colors cursor-pointer font-bold shadow-sm"
            >
              Reset Filters
            </button>
          </motion.div>
        ...
        <AnimatePresence>
          {activeProjectModal && (
            <div id="project-detail-modal" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
              <motion.div
